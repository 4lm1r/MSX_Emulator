...
void OpcodesHandler::executeOpcode(uint8_t opcode) {
    ...
    // Handle ED prefix for extended instructions (like RETI)
    if (opcode == 0xED) {
    ...
    // Handle DD prefix (IX register instructions)
    else if (opcode == 0xDD) {
        uint8_t next_opcode = readMemory(cpu.PC + 1);
    ...
    cycles = 8; // Base cycle count for CB instructions (adjust per instruction)
}
    // Handle FD prefix (IY register instructions)
    else if (opcode == 0xFD) {
        uint8_t next_opcode = readMemory(cpu.PC + 1);
    ...
    else if (opcode == 0xFE) { // CP n (prioritized explicitly)
    ...
    else if ((opcode & 0xCF) == 0xC6 || (opcode & 0xCF) == 0xCE) {
    ...
    else if ((opcode & 0xC0) == 0x00) { // 00xx xxxx pattern
    ...
    // Arithmetic and logic (10xx xxxx)
        ...
        if (opcode == 0xA0 || opcode == 0xA2 || opcode == 0xAF || opcode == 0xB0 || 
            ...
            handleLogic(opcode);
                   ...
                   opcode == 0x7C || opcode == 0x93 || opcode == 0x9A || opcode == 0xBE) { // Added 0x7C (LD A, H)
    ...
    // Data transfer, arithmetic, control flow (00xx xxxx and 01xx xxxx)
    else {
        debug_log << "Routing to default block for opcode 0x" << std::hex << (int)opcode << std::dec << std::endl;
        if (opcode == 0x3E || opcode == 0x06 || opcode == 0x0E || opcode == 0x21 || opcode == 0x77 || 
        ...
        else if (opcode == 0x3C || opcode == 0x3D || opcode == 0x80 || opcode == 0x90 || 
                 opcode == 0x86 || opcode == 0x04 || opcode == 0x0C || opcode == 0x05 || 
                 opcode == 0x23 || opcode == 0x03 || opcode == 0x24 || opcode == 0x93 || 
                 opcode == 0x9A || opcode == 0xBE || opcode == 0x2C || opcode == 0x25) { // Added 0x2C (INC L), 0x25 (DEC H)
            debug_log << "Routing to handleArithmetic" << std::endl;
            handleArithmetic(opcode);
        }
...
void OpcodesHandler::handleIYInstructions(uint8_t opcode) {
    switch (opcode) {
        ...
        default:
            debug_log << "Unhandled IY instruction opcode 0x" << std::hex << (int)opcode << std::dec << std::endl;
            cpu.PC++;
            cycles = 4;
    }
}
...
void OpcodesHandler::handleDataTransfer(uint8_t opcode) {
    switch (opcode) {
        ...
        default:
            debug_log << "Unhandled data transfer opcode 0x" << std::hex << (int)opcode << std::dec << std::endl;
            cpu.PC++;
            cycles = 4;
    }
}
...
void OpcodesHandler::handleIXInstructions(uint8_t opcode) {
    switch (opcode) {
        ...
        default:
            debug_log << "Unhandled IX instruction opcode 0x" << std::hex << (int)opcode << std::dec << std::endl;
            cpu.PC++;
            cycles = 4;
    }
}
